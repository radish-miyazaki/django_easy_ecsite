...
class ConfirmOrderView(LoginRequiredMixin, TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if (not address) or (not cart) or (not total_price):
            raise Http404('Error is occurred in order process')
...
def add_product(request):
    if request.is_ajax:
        ...
        if int(quantity) > product.stock:
            response = JsonResponse({'message': '在庫数を超過しています。'})
            response.status_code = 403
            return response

        if int(quantity) <= 0:
            response = JsonResponse({'message': '0以上を入力してください。'})
            response.status_code = 403
            return response
